...
(define (thaw-car sx)
  (let ((x ((sized-v sx))))
    (let ((x-a (car x)) (x-d (cdr x)))
      (cons x-a (sized (min (+ (sized-s sx) (sized-s x-d)) max-size) (sized-v x-d))))))
...
(define (disj++ fs)
  (let loop ((f (car fs)) (fs (cdr fs)) (ys '()))
    (cond
      ((not f)
       (cond
         ((>= (length ys) disj-v-max) (let-values ([(h t) (split-at ys disj-v-max)])
                                        (loop #f fs (cons (disj+ h) t))))
         ((null? fs) (conj-v ys))
         (else (loop (car fs) (cdr fs) ys))))
...
(define (conj++ fs)
  (let loop ((f (car fs)) (fs (cdr fs)) (ys '()))
    (cond
      ((not f)
       (cond
         ((>= (length ys) conj-v-max) (let-values ([(h t) (split-at ys conj-v-max)])
                                        (loop #f fs (cons (conj+ h) t))))
         ((null? fs) (conj-v ys))
         (else (loop (car fs) (cdr fs) ys))))
